...
def testRealEven(x):
    ...
    X = fft(fftbuffer)                                
    X[np.argwhere(abs(np.imag(X))<1e-6)]= np.real(X[np.argwhere((abs(np.imag(X))<1e-6))])
    Xshifted = fftshift(X)	
    isRealEven = np.all(np.isreal(X)) and np.all(abs(Xshifted[:]) == abs(Xshifted[::-1]))
	
    return bool(isRealEven), fftbuffer, X
