...
def zpFFTsizeExpt(x, fs):
    ...
    # To generate the example: x = A*np.cos(2*np.pi*f * np.arange(0,t,1/fs) + phi)...
    # where... A=1, f=110, fs=1000, t=M/fs=512/1000, phi=0
	
    #plt.plot(np.array(range(0,len(f1)))*fs/M,f1, 'b--o')
    #plt.plot(np.array(range(0,len(f2)))*fs/(2*M),f2, 'r--*')
    #plt.plot(np.array(range(0,len(f3)))*fs/(2*M),f3, 'g--x')
    
    return mX1[:qSamples], mX2[:qSamples], mX3[:qSamples]
